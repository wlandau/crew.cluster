# **\[maturing\]** Create a launcher with SLURM workers.
...
- options_cluster:
  ...
  An options list from [`crew_options_slurm()`](crew_options_slurm.md)
  with cluster-specific configuration options.
...
To launch a SLURM worker, this launcher creates a temporary job script
with a call to
...
It has all the lines except for the job name and the call to
[`crew::crew_worker()`](https://wlandau.github.io/crew/reference/crew_worker.html),
